...
public partial class AdonisWindow : Window
{
    ...
    {
        if (d is not AdonisWindow sourceWindow)
            return;
    ...
    private BitmapSource? GetApplicationIcon()
    {
        ...
        var appIcon = System.Drawing.Icon.ExtractAssociatedIcon(appFilePath);
        if (appIcon == null)
            return null;
    ...
    private void RestoreOnMouseMove(object sender, MouseEventArgs e)
    {
        ...
        var positionOnScreen = PointToScreen(positionInWindow);
        var currentScreen = ScreenInterop.FromPoint(positionOnScreen);
        var restoreSizeOnScreen = TransformToScreenCoordinates(new Size(RestoreBounds.Width, RestoreBounds.Height));
    ...
    {
        if (GetTemplateChild("TitleBar") is not Border titleBar)
            return;
