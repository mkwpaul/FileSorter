...
using BetterMessageBox.Control;
using System;
...
public class MessageBoxWindow<T> : AdonisWindow
{
    private const string PART_MessageBoxButtonContainer = "PART_MessageBoxButtonContainer";

    public IMessageBoxModel<T> ViewModel
    {
        get => DataContext as IMessageBoxModel<T>;
    ...
    /// A <see cref="Style"/> targeting <see cref="Button"/> that represents the style of all buttons inside the message box.
    /// </summary>
    public static readonly DependencyProperty ButtonStyleProperty = DependencyProperty.Register(nameof(ButtonStyle), typeof(Style), typeof(MessageBoxWindow<T>), new PropertyMetadata(null));

    /// <summary>
    /// A <see cref="Style"/> targeting <see cref="CheckBox"/> that represents the style of all check boxes inside the message box.
    /// </summary>
    public static readonly DependencyProperty CheckBoxStyleProperty = DependencyProperty.Register(nameof(CheckBoxStyle), typeof(Style), typeof(MessageBoxWindow<T>), new PropertyMetadata(null));

    /// <summary>
    /// A <see cref="Style"/> targeting <see cref="Border"/> that represents the style of the button container row.
    /// </summary>
    public static readonly DependencyProperty ButtonContainerStyleProperty = DependencyProperty.Register(nameof(ButtonContainerStyle), typeof(Style), typeof(MessageBoxWindow<T>), new PropertyMetadata(null));

    /// <summary>
    /// A <see cref="double"/> that represents the maximum width of the message box relative to the current screen width.
    /// </summary>
    public static readonly DependencyProperty MaxRelativeScreenWidthProperty = DependencyProperty.Register(nameof(MaxRelativeScreenWidth), typeof(double), typeof(MessageBoxWindow<T>), new PropertyMetadata(0.9));

    /// <summary>
    /// A <see cref="double"/> that represents the maximum height of the message box relative to the current screen height.
    /// </summary>
    public static readonly DependencyProperty MaxRelativeScreenHeightProperty = DependencyProperty.Register(nameof(MaxRelativeScreenHeight), typeof(double), typeof(MessageBoxWindow<T>), new PropertyMetadata(0.9));

    /// <summary>
    /// A <see cref="double"/> that represents the amount of which the maximum width is increased in case it does not fit the screen.
    /// The message box will choose the smallest multiple of this value where its height does not become larger than specified via <see cref="MaxRelativeScreenHeightProperty"/>.
    /// </summary>
    public static readonly DependencyProperty MaxWidthStepProperty = DependencyProperty.Register(nameof(MaxWidthStep), typeof(double), typeof(MessageBoxWindow<>), new PropertyMetadata(200.0));
    ...
    {
        DefaultStyleKeyProperty.OverrideMetadata(typeof(MessageBoxWindow<>), new FrameworkPropertyMetadata(typeof(MessageBoxWindow<T>)));
    }
    ...
    protected virtual bool CanCancel()
    {
        ...
        if (ViewModel.Buttons is not IEnumerable<IButtonModel<MessageBoxResult>> buttons)
            return false;

        if (buttons.Any(btn => btn.CausedResult == MessageBoxResult.OK || btn.CausedResult == MessageBoxResult.Cancel))
            return true;
    ...
    {
        var buttonModel = (IButtonModel<T>) ((FrameworkElement) sender).DataContext;

        if (ViewModel != null)
        {
            ViewModel.Result = buttonModel.Id;
    ...
    private void OnClosing(object sender, CancelEventArgs e)
    {
        ...
        if (ViewModel.ButtonPressed.CausedResult == MessageBoxResult.None)
        {
            if (!CanCancel())
                e.Cancel = true;
            else
            {
                var button = GetDefaultCancelResult();
                ViewModel.Result = button.Id;
            }
        }
    }
    ...
    protected virtual IButtonModel<T> GetDefaultCancelResult()
    {
        if (ViewModel == null)
            return new ButtonModel<T>("", MessageBoxResult.Cancel);

        var cancelResult = ViewModel.Buttons.FirstOrDefault(btn => btn.CausedResult == MessageBoxResult.Cancel);

        foreach(var button in ViewModel.Buttons)
        {
            if (button.CausedResult == MessageBoxResult.Cancel)
                return button;
        }

        foreach (var button in ViewModel.Buttons)
        {
            if (button.CausedResult == MessageBoxResult.OK)
                return button;
        }
